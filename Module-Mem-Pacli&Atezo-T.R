...
# The list of samples here includes blood samples treated with paclitaxel plus atezolizumab.
...
# The following set (commented out) refers to tumor samples treated with paclitaxel plus atezolizumab.
...
# A different set of tumor samples treated with paclitaxel, which is being used in this case.
...
for (i in seq_along(folder_names)) {
  ...
  # Adding metadata: identifying the sample origin and its condition (pre-treatment, post-treatment, or progressive)
  seurat_object$orig.ident <- folder_name
  seurat_object$sample <- tolower(strsplit(folder_name, "_")[[1]][1])
  
  # Storing each Seurat object in a list
  seurat_list[[folder_name]] <- seurat_object
}

# --------- Merge the Seurat objects from different samples into a single object
merged_samples <- merge(
  x = seurat_list[[1]],  # Start with the first Seurat object
  y = seurat_list[-1],   # Merge the rest of the Seurat objects
  add.cell.ids = names(seurat_list),  # Use the folder names as unique cell IDs
  project = "Tumor_samples"  # Set the project name for the merged object
)

# Prepare a list of sample identifiers, assign "1" for post-treatment and "2" for progressive samples
samples <- folder_names
post_samples <- samples[grepl("Post|Prog", samples)]
trait <- ifelse(grepl("Prog", post_samples), "2", "1")  # Set the trait value based on "Prog" for progressive samples
...
# --------- Pseudo bulk transformation: Summing gene counts across cells by sample
...
head(pseudo_bulk)  # Preview the aggregated data
dim(pseudo_bulk)  # Check the new dimensions (genes x sample groups)

# Normalize by the number of cells in each sample
mean_counts <- sweep(pseudo_bulk, 2, cell_counts, FUN = "/")  # Normalize by cell count
head(mean_counts)
dim(mean_counts)

# Update the pseudo_bulk matrix with normalized counts
pseudo_bulk <- mean_counts
rm(mean_counts)
rm(raw_counts)
rm(combined)

# --------- Split the samples into pre-treatment and post-treatment groups for constructing two WGCNA networks
sample_names <- colnames(pseudo_bulk)
sample_group <- ifelse(grepl("^Pre", sample_names), "pre", "post")  # Categorize samples into "pre" or "post"
table(sample_group)

pre_samples <- colnames(pseudo_bulk)[sample_group == "pre"]  # Pre-treatment samples
post_samples <- colnames(pseudo_bulk)[sample_group == "post"]  # Post-treatment samples
...
# ---------- Variance filtering: Remove genes with zero variance in both pre and post groups
var_pre  <- apply(datExpr_pre, 2, var)
var_post <- apply(datExpr_post, 2, var)

# Identify genes with zero variance in both pre and post-treatment groups and exclude them
...
# ---------------------------
# Read gene information (e.g., the top 10 clusters)
data1 <- read.csv("D:\\Thesis\\Proposal\\Analysis\\WGCNA\\Blood-Patient\\Blood-Patient-Paclitaxel-Analysis\\TOP10-Clusters-final-Blood-Pacli.csv")
...
# Extract gene names from the data
...
head(MEs_pr)  # Preview the module eigengenes
module_names <- colnames(MEs_pr)  # List of module names
module_names

# Common Gene Selection: Subset the data to include only the selected genes
genes_to_use <- intersect(gene_info, colnames(datExpr_pre))  # Select genes common to both the gene info and expression data
datExpr_pre1 <- datExpr_pre[, genes_to_use]  # Subset the expression data
...
MEs_mat <- MEs_pr[common_samples, ]  # Subset eigengene matrix
expr_mat <- datExpr_pre1[common_samples, ]  # Subset gene expression matrix

# Check the number of common samples
length(common_samples)
head(rownames(MEs_pr))  # Preview sample names in eigengene matrix
head(rownames(datExpr_pre1))  # Preview sample names in expression matrix
...
# Find the maximum membership value for each gene
